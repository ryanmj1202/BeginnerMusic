...
import { toStoredNote } from './features/editor/utils/noteUtils'
import { useProjectStorage } from './features/editor/hooks/useProjectStorage'
import { useProjectHistory } from './features/editor/hooks/useProjectHistory'
import { useTrackActions } from './features/editor/hooks/useTrackActions'
import { useTempoActions } from './features/editor/hooks/useTempoActions'
import { useAutoMixActions } from './features/editor/hooks/useAutoMixActions'
import { useArrangementActions } from './features/editor/hooks/useArrangementActions'
import { useKeyboardRecording } from './features/editor/hooks/useKeyboardRecording'
import { usePreviewNote } from './features/editor/hooks/usePreviewNote'
import { usePianoRollSelection } from './features/editor/hooks/usePianoRollSelection'
import { usePianoRollDrag } from './features/editor/hooks/usePianoRollDrag'
import { usePlayback } from './features/editor/hooks/usePlayback'
import { useFileActions } from './features/editor/hooks/useFileActions'
import { TopMenu } from './features/editor/components/TopMenu'
import { TrackPanel } from './features/editor/components/TrackPanel'
import { InstrumentDialog } from './features/editor/components/InstrumentDialog'
import { TempoPanel } from './features/editor/components/TempoPanel'
import { AutoMixPanel } from './features/editor/components/AutoMixPanel'
import { ArrangeView } from './features/editor/components/ArrangeView'
import { DetailPanel } from './features/editor/components/DetailPanel'
import { PianoRollView } from './features/editor/components/PianoRollView'
...
function App() {
  ...
  const {
    ...
    keyPreviewRef,
    previewTokenRef,
    selectedInstrumentId: selectedTrack?.instrumentId,
    setPressedPitch,
  })
  const {
    beginLassoSelection,
  ...
  return (
    ...
    >
      <TopMenu
        ...
        cutSelectedNotes={cutSelectedNotes}
        duplicateSelectedNotes={duplicateSelectedNotes}
        editMenuOpen={editMenuOpen}
        fileInputRef={fileInputRef}
        fileMenuOpen={fileMenuOpen}
        hasPatternClipboard={Boolean(patternClipboardRef.current)}
        importAudioFiles={importAudioFiles}
        isExportingMp3={isExportingMp3}
        loadProjectFile={loadProjectFile}
        openProjectFile={openProjectFile}
        pasteSelectedNotes={pasteSelectedNotes}
        projectTitle={project.title}
        redoProject={redoProject}
        saveMidiFile={saveMidiFile}
        saveMp3File={saveMp3File}
        saveProjectFile={saveProjectFile}
        selectedPatternNoteCount={selectedPatternNotes.length}
        setActiveEditorTab={setActiveEditorTab}
        setAutoMixPanelOpen={setAutoMixPanelOpen}
      ...
      <InstrumentDialog
        ...
        closeInstrumentDialog={closeInstrumentDialog}
        confirmInstrumentDialog={confirmInstrumentDialog}
        instrumentCategories={instrumentCategories}
        instrumentCategory={instrumentCategory}
        instrumentDialogTrack={instrumentDialogTrack}
        previewInstrumentChoice={previewInstrumentChoice}
      ...
      <main className="editor-shell">
        <TrackPanel
          addAutoMixSection={addAutoMixSection}
          addTrack={addTrack}
          allTrackMelodyMode={allTrackMelodyMode}
          applyAutoMixGenrePreset={applyAutoMixGenrePreset}
          autoMixGenrePreset={autoMixGenrePreset}
          autoMixPanelOpen={autoMixPanelOpen}
          autoMixReport={autoMixReport}
          deleteAutoMixSection={deleteAutoMixSection}
          focusAutoMixSection={focusAutoMixSection}
          getAutoMixFocusTrackId={getAutoMixFocusTrackId}
          ...
          openTrackPanelContextMenu={openTrackPanelContextMenu}
          project={project}
          runAutoMixTracks={runAutoMixTracks}
          selectTrack={selectTrack}
          selectedAutoMixSectionId={selectedAutoMixSection?.id ?? null}
          selectedTrackName={selectedTrack?.name ?? '트랙 1'}
          setAllTrackMelodyMode={setAllTrackMelodyMode}
          setAutoMixFocusTrack={setAutoMixFocusTrack}
          setAutoMixPanelOpen={setAutoMixPanelOpen}
          toggleVoiceRecording={toggleVoiceRecording}
          totalBeats={totalBeats}
          updateAutoMixSection={updateAutoMixSection}
        ...
        <section className="piano-roll-area" aria-label="피아노 롤">
          {activeEditorTab === 'piano-roll' ? (
            <PianoRollView
              ...
              patternRepeatGapInput={patternRepeatGapInput}
              pianoRollRef={pianoRollRef}
              playbackPressedPitchCounts={playbackPressedPitchCountsRef.current}
              pressedPitch={pressedPitch}
              projectSelectedNoteId={project.selectedNoteId}
              renderAutoMixCutOverlay={renderAutoMixCutOverlay}
              ...
              selectedPatternRepeatGroup={selectedPatternRepeatGroup}
              selectedTrack={selectedTrack}
              selectedTrackIsAudio={selectedTrackIsAudio}
              selectionBox={selectionBox}
              setKeyboardInputEnabled={setKeyboardInputEnabled}
              setNoteDivision={setNoteDivision}
              setPatternRepeatGapInput={setPatternRepeatGapInput}
              startResizingNote={startResizingNote}
              toolMode={toolMode}
              totalBeats={totalBeats}
              ungroupSelectedPattern={ungroupSelectedPattern}
              visibleBars={visibleBars}
              zoomRoll={zoomRoll}
            />
          ) : activeEditorTab === 'arrange' ? (
            <ArrangeView
              addAutoMixSectionFromPointer={addAutoMixSectionFromPointer}
              addTrackPlacement={addTrackPlacement}
              arrangedProject={arrangedProject}
              beginTrackPlacementDrag={beginTrackPlacementDrag}
              deleteTrackPlacement={deleteTrackPlacement}
              focusTrackAtBeat={focusTrackAtBeat}
              project={project}
              renderAutoMixCutOverlay={renderAutoMixCutOverlay}
              rollSurfaceStyle={rollSurfaceStyle}
              selectTrack={selectTrack}
              selectedTrackId={selectedTrack?.id}
              selectedTrackPlacementId={selectedTrackPlacementId}
              totalBeats={totalBeats}
              visibleBars={visibleBars}
            />
          ) : activeEditorTab === 'tempo' ? (
            <TempoPanel
              addTempoSection={addTempoSection}
              changeTempoFromGraph={changeTempoFromGraph}
              changeTempoInput={changeTempoInput}
              commitTempoInput={commitTempoInput}
              deleteTempoSection={deleteTempoSection}
              focusTempoSection={focusTempoSection}
              projectTempo={project.tempo}
              renderAutoMixCutOverlay={renderAutoMixCutOverlay}
              renderTempoSectionOverlay={renderTempoSectionOverlay}
              rollSurfaceStyle={rollSurfaceStyle}
              selectedTempoSection={selectedTempoSection}
              setSelectedTempoSectionId={setSelectedTempoSectionId}
              tempoInput={tempoInput}
              tempoSections={tempoSections}
              updateTempo={updateTempo}
              updateTempoSection={updateTempoSection}
            />
          ) : (
            <AutoMixPanel
              addAutoMixSection={addAutoMixSection}
        ...
        <DetailPanel
          activeDetailTerm={activeDetailTerm}
          activeNoteControl={activeNoteControl}
          beginDetailGraphDrag={beginDetailGraphDrag}
          changeTempoInput={changeTempoInput}
          commitTempoInput={commitTempoInput}
          detailGraphBounds={detailGraphBounds}
          detailGraphDragRef={detailGraphDragRef}
          detailGraphNotes={detailGraphNotes}
          detailGraphSvgRef={detailGraphSvgRef}
          detailPanelOpen={detailPanelOpen}
          editableSelectedNotes={editableSelectedNotes}
          finishDetailGraphDrag={finishDetailGraphDrag}
          getNoteControlValue={getNoteControlValue}
          getSelectedNoteValue={getSelectedNoteValue}
          isPlaying={isPlaying}
          projectTempo={project.tempo}
          resetPlayback={resetPlayback}
          selectedNote={selectedNote}
          setActiveDetailTerm={setActiveDetailTerm}
          setDetailPanelOpen={setDetailPanelOpen}
          snapBeatToGrid={snapBeatToGrid}
          sortedEditableSelectedNotes={sortedEditableSelectedNotes}
          tempoInput={tempoInput}
          togglePlayback={togglePlayback}
          updateDetailGraphFromPointer={updateDetailGraphFromPointer}
